...
- **`hal_master_probe`**: Checks if the device exists on the master HAL communication bus using `i2c_master_probe`. Accepts a `void*` HAL master communication bus handle.
- **`hal_master_init`**: Configures the device (address, clock speed) and adds it to the HAL master communication bus using `i2c_master_bus_add_device`. Accepts a `void*` HAL master communication bus handle.
- **`hal_master_read`**: Wraps `i2c_master_receive` to read data from the sensor. Accepts a `void*` HAL device communication handle.
- **`hal_master_write`**: Wraps `i2c_master_transmit` to write data (commands) to the sensor. Accepts a `void*` HAL device communication handle.
- **`hal_master_write_command`**: A specialized write function for sending single-byte commands. Accepts a `void*` HAL device communication handle.
- **`hal_master_remove`**: Removes the device from the I2C bus using `i2c_master_bus_rm_device`. Accepts a `void*` HAL device communication handle.
