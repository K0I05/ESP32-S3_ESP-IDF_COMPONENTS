...
[![ESP Component Registry](https://components.espressif.com/components/k0i05/esp_pressure_tendency/badge.svg)](https://components.espressif.com/components/k0i05/esp_pressure_tendency)

This ESP32 espressif IoT development framework (esp-idf) pressure tendency component observes the last 3 or 6 hours of atmospheric pressure samples to determine if pressure is steady, rising, or falling.  Information on features and functionality are documented and can be found in the `pressure_tendency.h` header file.
...
To get started, simply copy the component to your project's `components` folder and reference the `pressure_tendency.h` header file as an include.  The component includes documentation such as the readme file, application notes, and/or user manual where applicable.
...
void tendency(void) {
    /* attempt to initialize a pa 3-hr tendency handle */
    pressure_tendency_init(tendency_sampling_interval, PRESSURE_TENDENCY_3HR_PERIOD, &pa_tendency_hdl);
